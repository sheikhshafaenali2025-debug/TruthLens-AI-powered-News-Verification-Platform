  ...
  function analyzeText(text, sourceRaw) {
    ...
    const noise = (seed % 12) / 100;

    let fakeLean = 0.28 + hedgeScore * 0.09 - credScore * 0.07 + noise;
    if (words.length < 25) fakeLean += 0.08;
    fakeLean = Math.max(0.03, Math.min(0.94, fakeLean));

    const isFake = fakeLean >= 0.5;
    const realPct = Math.round((1 - fakeLean) * 100);
    const fakePct = 100 - realPct;
    const confidence = isFake ? fakePct : realPct;
  ...
  analyzeBtn.addEventListener('click', () => {
    ...
    const interval = setInterval(() => {
      if (step < totalSteps) {
        ...
        setTimeout(() => runResults(text, sourceInput.value), 250);
      }
    }, stepTime);
  });

  function runResults(text, sourceRaw) {
    const r = analyzeText(text, sourceRaw);
    ...
    document.getElementById('procTime').textContent = (0.28 + (r.wordCount % 20) / 100).toFixed(2) + 's';
